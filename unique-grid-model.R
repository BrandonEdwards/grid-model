...
for (i in 5:length(grid))
{
  ...
  HER.ANALYZE<-HER.MERGED[HER.MERGED$GRID %in% c(grid[i], grid[i-1], grid[i-2], grid[i-3], grid[i-4]), ]
  ...
  pdf(paste("C:/Users/Brandon/Documents/GitHub/grid-model/Unique Grid Plots/Gelman_Plots_", region, ".pdf", sep=""), paper="a4")
  gelman.plot(samples)
  dev.off()
  ...
  write.csv(results.1, paste("C:/Users/Brandon/Documents/GitHub/grid-model/Unique Grid Plots/", region, " parameter estimates.csv", sep=""))
